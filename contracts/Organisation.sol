...
contract Organisation is Ownable(msg.sender) {
    // Employee structure
    struct Employee {
    ...
    event EmployeeAdded(
        ...
        uint256[2] publicViewerKey,
        uint256[2] publicSpenderKey
    );
    event EmployeeDeactivated(uint256 indexed employeeId);
    event EmployeeKeysUpdated(
        uint256 indexed employeeId,
        uint256[2] newPublicViewerKey,
        uint256[2] newPublicSpenderKey
    );
    ...
    function updateEmployeeKeys(
        ...
        require(
            employees[_employeeId].employeeId != 0,
        ...
        emit EmployeeKeysUpdated(
            _employeeId,
            _newPublicViewerKey,
            _newPublicSpenderKey
        );
    }
    ...
    function dispatchPayment(
        ...
        require(
            commitmentToPayment[commitment].hash == bytes32(0),
        ...
        if (chain_id == ROOT_STOCK_CHAIN_ID) {
            require(
                IERC20(_token).transfer(stealth_address, _amount),
    ...
    function updateCommitmentTransactionHash(
        ...
        require(
            commitmentToPayment[_commitment].hash != 0,
